...
abstract contract BaseRouter is Initializable {
    ...
    function mixer( //@audit-issue MED reentrancy
        ...
        console.log("inside mixer");
        // Performing actions one after the others
        uint256 actionsLength = actions.length;
        for (uint256 i; i < actionsLength; ++i) {//@audit can we frontrun allowances to steal all users allowed assets?
    ...
    function _angleBorrower(
        ...
        address who,
        bytes memory repayData
    ) internal virtual returns (PaymentData memory paymentData) {//@audit-ok this always send from msg.sender, but allowance is not decreased in current version I think. So if anyone approves anything to vault, can we get the money? - there are checks inside _checkVaultID
    ...
    function _safeTransferNative(address to, uint256 amount) internal {
        ...
        assembly {
            ...
            success := call(gas(), to, amount, 0, 0, 0, 0)//audit-issue MED gas griefing
        }
    ...
    function _parseVaultIDs(//@audit take a closer look, seems smelly
        ActionBorrowType[] memory actionsBorrow,
        bytes[] memory dataBorrow,
        address vaultManager,//@audit user-provided input, can we break it?
        address collateral
    ) internal view returns (bytes[] memory) {

    /*@audit  possible actions:
        createVault,
        closeVault,
        addCollateral,
        removeCollateral,
    ...
    */
        ...
        for (uint256 i; i < actionsBorrowLength; ++i) {
            ...
            console.log("create vault action=", createVaultAction);
            ...
            if (actionsBorrow[i] == ActionBorrowType.createVault) {
                ...
                console.log("create, no vault override");
                continue;
                ...
                if (amount == type(uint256).max)
                    dataBorrow[i] = abi.encode(vaultID, IERC20(collateral).balanceOf(address(this)));//@audit-info if I get this correctly, dataBorrow is set but not used - it's just a safeguard - modifies input dataBorrow and returns fixed version of it
                continue;
            ...
            console.log("action", uint8(actionsBorrow[i]));
            if (vaultID == 0) {
                if (createVaultAction) {
                    ...
                    if (lastVaultID == 0) {//@audit-info VaultManager::createVault user _mint, which checks if sender can receive NFT - it can be used to reenter this router (VaultManager has nonReentrant protection)
                        lastVaultID = IVaultManagerStorage(vaultManager).vaultIDCount();//@audit-ok possible issue, someone can frontrun it to get others' debt - not applicable due to createVaultAction
                        console.log("overriding vaultID to ", lastVaultID);
                    }
                    vaultID = lastVaultID;
                }
            }
            ...
            for (uint256 j; j < vaultIDLength; ++j) {
                if (vaultIDsToCheckOwnershipOf[j] == vaultID) {//@audit-ok vaultIDsToCheckOwnershipOf is always 0, as it's not set anywhere. - it's set below, but I don't know why
                    // If yes, we continue to the next iteration
                    continue;
                }
            }
            // Verify this new `vaultID` and add it to the list
            if (!IVaultManagerFunctions(vaultManager).isApprovedOrOwner(msg.sender, vaultID)) {//@audit would be great if we could skip this check
